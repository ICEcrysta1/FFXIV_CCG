...
# 4) 启动工具菜单：输入 1～8 执行对应工具，0 退出
.\ffxiv_ccg.ps1
```

菜单中的 BC 训练、恢复训练和 GRPO 会共用 TensorBoard Web 监控启动入口；菜单第 8 项或 `-Action monitor` 可单独打开监控。服务就绪后会打开本机浏览器，端口由 `.env` 的 `TENSORBOARD_PORT` 指定。若该端口上的旧服务读取其他模型的事件目录，启动器会报错并提示关闭旧服务或换端口。
