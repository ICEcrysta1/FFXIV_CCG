...
- 清理 checkpoint 默认配置：菜单 2～6 直接传递所选 checkpoint，`.env` 不再需要维护 `TRAINING_MODEL_CHECKPOINT`、`AUTOREGRESSIVE_REPLAY_CHECKPOINT` 或 `AUTOREGRESSIVE_REPLAY_ONNX_PACKAGE`；未显式选择时回退到模型输出目录的 `best.pt`，ONNX 部署包按 checkpoint 自动映射。仍设置这些环境变量时按显式覆盖处理，命令行显式传入的 checkpoint 优先。
