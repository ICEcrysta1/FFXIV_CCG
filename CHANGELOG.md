...
- 将副本/百分位目录约定抽到 `scripts/common/dataset_layout.py`，下载器共用区间名称、分档与目录构造，转换脚本共用 JSON 递归发现；补充保留副本、区间和文件名的跨数据阶段路径映射，供后续评分或独立文件导出复用。现有下载目录、分档配额、文件发现与按副本选择行为保持不变，compiled cache 保存布局不变；同步补充目录映射、旧目录兼容测试和使用文档。
