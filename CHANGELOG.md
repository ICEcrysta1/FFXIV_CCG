...
- 新增 `scripts.action_quality` 离线动作质量桥接入口：复用根目录 `.env`、模型等级权重和公共数据集目录映射，调用独立 Node 解析进程，在 `annotated/<副本>/<百分位区间>/` 保存保留原始字段与事件的 JSON，并在顶层 `analysis` 输出整场建议、动作引用、插入窗口和黑魔循环证据。按输入职业选择解析模块，补充机工证据提取；保留事件匹配状态，逐动作严重度未确定时不套用整场等级。本次尚未接入训练损失或 PPG 权重，产物明确标记 `training_ready: false`。
...
- FFLogs 批量下载改为按历史百分位十档均分总目标份数，新增 `--count`（默认 `200`）与 `--partition`；默认使用 API 默认分区，显式指定分区时榜单与角色历史查询保持一致，不再只下载个人最佳记录。输出按 `raw/<副本>/<区间>/` 分类，最低档为 `00-10`；JSON 新增 `ranking` 元数据，明确历史排名百分位与训练 PPG 分位数的区别。
- 分档下载仅将符合当前身份、排名口径和完整性要求的已有文件计入配额，失败继续同档补位，候选不足时报告缺额而不挪用其他档位；补充配额、区间边界、API 分区、分页与文件复用测试，并更新命令示例和项目结构说明。
...
- 训练新增 `training.max_files`：在模型 YAML 中限制参与训练与验证的 raw JSON 文件数，只接受正整数或 `null`（浮点、字符串等非整数取值直接报错，不做静默截断），`null` 表示使用 raw 目录下全部有效文件；选择时按副本子目录比例分配，再按 `train_split` 划分训练集与验证集。一键脚本无需额外参数即可控制数据规模，命令行 `--max-files` 仍然优先并把最终生效值写回 `RunConfig`，使 checkpoint 的 `run_config.max_files` 记录本次生效的数据上限，启动日志会打印实际取值与来源；当前黑魔 `artzip` 配置固定为 `1280`。
